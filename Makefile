...
ARCH_NUM ?= 70
PREFIX ?= $(shell pwd)
NVCC_C_OPTS = -keep -rdc=true --generate-line-info -std=c++17 -gencode arch=compute_$(ARCH_NUM),code=sm_$(ARCH_NUM) -lcudart -lcublas -Xcompiler -fPIC -Xcompiler -fopenmp -Xcompiler -O3 -Xcompiler -g -Xcompiler -Wall
