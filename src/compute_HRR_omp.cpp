...
void execute_HRR1_gpu(
   ...
   for( int ikl=0 ; ikl < NcoA*NcoC*NcoD ; ikl++ ){
      int l = ikl%NcoD;
      int k = (ikl/NcoD)%NcoC;
      int i = ikl/NcoD/NcoC;
      int kp, lm, d ;
...
void execute_HRR2_gpu(
   ...
   #pragma omp parallel for
   for( int ijkl=0 ; ijkl < NcoABCD ; ijkl++ ){
...
//      const int tid = omp_get_thread_num(); // threadIdx.x;
//      const int Nt = omp_get_num_threads(); // blockDim.x;
